...
class Utility
{
	...
	public static function formatSeconds($seconds)
	{
		if ($seconds == 0) {
			...
			$currentTime = time();
			$time = Utility::distance_of_time_in_words($currentTime,
				$currentTime + $seconds, true);
		}
	...
	public static function distance_of_time_in_words($from_time, $to_time = null, $include_seconds = false)
	{
		...
		$distance_in_minutes = floor($distance_in_seconds / 60);
		...
		if ($distance_in_minutes <= 1) {
			if (!$include_seconds) {
				...
				} else if ($distance_in_seconds >= 20 && $distance_in_seconds <= 40) {
		...
		} else if ($distance_in_minutes >= 2 && $distance_in_minutes <= 59) {
