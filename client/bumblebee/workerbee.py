class WorkerBee():
  ...
  def startup(self):
    ...
    if (self.data['status'] == 'working' or self.data['status'] == 'finished')
      ...
      if (result['status'] == 'success'):
        ...
        raise new Exception("Unable to clear stale job.")
  ...
  def driverFactory(self, config):
    if (self.config['driver'] == 's3g'):
      return new BQS3GDriver(self.config);
    elif (self.config['driver'] == 'passthru'):
      return new BQSerialPassthruDriver(self.config)
    else:
      raise new Exception("Unknown driver specified.")
    ...
    if (result['status'] == 'success'):
      if (len(result['data']))
        ...
        if (jresult['status'] == 'success'):
          ...
          raise new Exception("Error grabbing job.")
      else:
        sleep(10) #todo: make this sleep get longer with each successive try.
    else:
      raise new Exception("Error listing jobs in queue.")
