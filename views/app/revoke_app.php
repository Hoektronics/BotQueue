	...
	<form class="form-horizontal" method="post" autocomplete="off" action="<?=$token->getUrl()?>/revoke">
		...
		<div class="alert alert-block">
			<h4 class="alert-heading">Warning!</h4>
				Are you sure you want to <?= ($token->get('type') == 2) ? 'revoke' : 'deny' ?> access to the <?=$token->getLink()?> app?  <? if ($token->get('type') == 2): ?>Any apps currently using these credentials to print will be broken.<? endif ?>
		</div>
