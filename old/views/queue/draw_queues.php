	...
	<table class="table table-striped table-bordered table-condensed">
		...
		<? foreach ($queues AS $row): ?>
			...
			<tr>
				<td><?= $q->getLink() ?></td>
				<td><span
						class="label <?= JobStatus::getStatusHTMLClass('available') ?>"><?= (int)$stats['available'] ?></span>
				</td>
				<td><?= round($stats['available_pct'], 2) ?>%</td>
				<td><span
						class="label <?= JobStatus::getStatusHTMLClass('slicing') ?>"><?= (int)$stats['slicing'] ?></span>
				</td>
				<td><?= round($stats['slicing_pct'], 2) ?>%</td>
				<td><span class="label <?= JobStatus::getStatusHTMLClass('taken') ?>"><?= (int)$stats['taken'] ?></span>
				</td>
		...
		<? if (count($queues) > 1): ?>
			<tr>
				<th>Total</th>
				<th><span
						class="label <?= JobStatus::getStatusHTMLClass('available') ?>"><?= (int)$total['available'] ?></span>
				</th>
				<th><?= round(($total['available'] / $total['total']) * 100, 2) ?>%</th>
				<th><span
						class="label <?= JobStatus::getStatusHTMLClass('slicing') ?>"><?= (int)$total['slicing'] ?></span>
				</th>
				<th><?= round(($total['slicing'] / $total['total']) * 100, 2) ?>%</th>
				<th><span class="label <?= JobStatus::getStatusHTMLClass('taken') ?>"><?= (int)$total['taken'] ?></span>
				</th>
				<th><?= round(($total['taken'] / $total['total']) * 100, 2) ?>%</th>
				<th><span
						class="label <?= JobStatus::getStatusHTMLClass('complete') ?>"><?= (int)$total['complete'] ?></span>
				</th>
				<th><?= round(($total['complete'] / $total['total']) * 100, 2) ?>%</th>
				<th><span
						class="label <?= JobStatus::getStatusHTMLClass('failure') ?>"><?= (int)$total['failure'] ?></span>
				</th>
